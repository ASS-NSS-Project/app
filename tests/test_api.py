"""
Backend API and chunking tests.

Coverage:
- Service-level endpoints: `/health` and `/metrics`.
- Auth flow behavior: invalid login, valid login, `/auth/me`, `/auth/stats`.
- Authorization guards on protected routes (`/sources`, `/query`, `/documents`, `/incidents`).
- Basic authenticated list-route behavior for sources/documents/incidents.
- SSRF protection check in source creation.
- Chunking unit tests for prose, table, and VLM block splitters.
"""
import pytest
